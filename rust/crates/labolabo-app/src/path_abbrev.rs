...
/// 省略を始める表示長のしきい値(文字数 -- バイトではない)。サイドバー幅の
/// 既定値(`sidebar::DEFAULT_SIDEBAR_WIDTH` = 220px。第16波 #1 で
/// ドラッグ可変になったが、この定数は変わらず既定幅を前提に選んだ値の
/// まま)と `font_size::CAPTION`(11px)を前提に 32 字(指示の「32〜40 字
/// 目安」の下限 -- 想定ユースケースの `~/ghq/github.com/owner/repo` 級=
/// 34 字前後がちゃんと省略対象に入る値)。
