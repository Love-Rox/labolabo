...
///
/// ## [`ACCENT`](青) と [`BRAND`](ライム) の使い分け (第10波)
///
/// - **`ACCENT` = 「いまの操作対象」**: ペインのフォーカス枠・グロー、
///   選択中のタブ下線、端末の選択ハイライト、タブ移動 DnD など、
///   キーボード入力が向かう先/操作の焦点を示す従来用途はすべて青のまま。
/// - **`BRAND` = 「選択中のタスク」**: サイドバーで選ばれているタスク行の
///   左バー + 低アルファ背景だけに使う LaboLabo のブランドライム
///   (サイトのブランドトークンと同値)。「どのタスクを開いているか」と
///   「どのペインにフォーカスがあるか」は別のレイヤーの答えなので、
///   色も分ける -- 両方を青にすると一瞥で区別できない。
pub const ACCENT: u32 = 0x5e9eff;

/// LaboLabo ブランドカラー(ライム)。選択中タスクの強調専用 --
/// 使い分けの原則は [`ACCENT`] の doc コメント参照。
pub const BRAND: u32 = 0xd0ff00;

/// [`BRAND`] の一段沈んだ変種。ブランド色を面ではなく細部(テキスト入力の
/// 枠線など、主張しすぎては困るがブランドの系譜は保ちたい箇所)に使う。
pub const BRAND_DIM: u32 = 0xa9d400;
