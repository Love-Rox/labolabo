...
impl<T: EventListener> Handler for Term<T> {
    ...
    fn push_keyboard_mode(&mut self, mode: KeyboardModes) {
        ...
        // LOCAL PATCH (LaboLabo, W17): upstream copy-pasted this bound-check
        // branch from `push_title` (below) and never updated which stack it
        // trims -- `self.title_stack.remove(0)` where the surrounding
        // function, comment, and every other line in it operate on
        // `self.keyboard_mode_stack`. `title_stack` starts empty and this
        // function never pushes to it, so once `keyboard_mode_stack` reaches
        // `KEYBOARD_MODE_STACK_MAX_DEPTH` (4096) pushes with no intervening
        // pop -- reachable via 4096 uninterrupted `CSI > <flags> u`
        // sequences on a PTY with `kitty_keyboard: true` (this workspace's
        // own `AlacrittyBackend::new`, `crates/labolabo-term/src/backend/
        // alacritty.rs`) -- `Vec::remove(0)` on the still-empty `title_stack`
        // panics ("index out of bounds"), unwinding the worker thread
        // `run_worker` (`crates/labolabo-term/src/session.rs`) runs on and
        // permanently freezing that pane (no catch_unwind there). Fixed here
        // to trim `keyboard_mode_stack` itself, matching every other
        // reference to it in this function. Reported nowhere upstream yet
        // (github.com/alacritty/alacritty) -- pending a separate decision;
        // drop this patch once a fix lands there.
