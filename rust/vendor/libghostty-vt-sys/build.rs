...
fn build_vendored(link_mode: LinkMode) {
    ...
    // `HOST` is no longer read: -Dtarget is now passed unconditionally (see
    // the LOCAL PATCH comment below), so we don't need target != host to
    // decide whether to cross-compile.
    ...
    // Always pass -Dtarget, even for a "native" (target == host) build.
    //
    // LOCAL PATCH (LaboLabo): upstream skipped -Dtarget for native builds so
    // Zig would auto-detect the host, mirroring ghostty's own CMakeLists.txt.
    // But Zig's "no -Dtarget" resolution and an explicit "-Dtarget=<triple>"
    // resolution are NOT equivalent for the CPU model: omitting -Dtarget lets
    // Zig probe the exact build machine's CPU and bake in every extension it
    // finds (AVX-512, etc. -- confirmed via `-femit-llvm-ir` target-features
    // dumps), while an explicit target triple with no CPU suffix resolves to
    // the architecture's portable *baseline* CPU (same dump, all `-avx*`
    // etc.). Ghostty's own build.zig already special-cases native-macOS
    // builds to force a generic CPU model (`Config.genericMacOSTarget`,
    // workaround for github.com/mitchellh/ghostty/issues/1640) but has no
    // equivalent for Linux/other native targets, so a native x86_64 Linux
    // build bakes in the exact CPU features of whichever machine ran it.
    //
    // On a heterogeneous CI/build fleet (GitHub Actions runners differ in
    // CPU generation between runs) this produces a binary that SIGILLs the
    // moment it hits an instruction the *running* machine's CPU lacks --
    // observed as intermittent `signal: 4, SIGILL` crashes in
    // `labolabo-term`'s backend_common tests, worsened by `mlugg/setup-zig`
    // persisting `.zig-cache` across unrelated CI runs (a cache built by a
    // wide-ISA runner gets restored and reused by a narrower one). The same
    // native-CPU baking applies to release builds: an official Linux/macOS
    // bundle built on whatever CPU the release runner happens to have could
    // crash on an end user's older/different CPU.
    //
    // Forcing -Dtarget unconditionally makes every build -- native or cross
    // -- resolve to the portable baseline CPU for that architecture, which
    // is what we want for both CI reliability and distribution safety.
    // Tracking: this should be upstreamed to
    // https://github.com/uzaaft/libghostty-rs and this vendor copy dropped
    // once a fixed release is available (see rust/vendor/libghostty-vt-sys
    // README note).
