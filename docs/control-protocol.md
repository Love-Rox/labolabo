...
- **Status**: v1.1（2026-07-16。v1 = 2026-07-15 に Rust 新版 (`labolabo-core`/`labolabo-app`) の実装を仕様として固定。v1.1 で §9 の Windows Named Pipe トランスポートを「予約」から実仕様へ昇格）
...
- 実行チャネルは**同一ユーザーのみ**アクセス可能（AF_UNIX ソケット 0600 + 親ディレクトリ 0700 / Windows は Named Pipe の same-user DACL — §9）。
...
- **チャネル**: AF_UNIX（`SOCK_STREAM`）。hooks ソケットとは**別ソケット**（別ファイル）。アプリインスタンスごとに 1 本。
  - Windows は Named Pipe（本節の AF_UNIX 仕様に対応する差分を §9 に規定。フレーミングの意味論は同一）。
...
`LABOLABO_CONTROL_SOCKET` は、LaboLabo が生成した端末ペインで起動されるすべてのプロセスに、既存の `LABOLABO_PANE`/`LABOLABO_TASK`（docs/hooks-protocol.md §7）と同じ場所（PTY spawn 時の env ブロック）で注入される。値は unix では §3 のソケットパス、Windows では §9 のパイプ名（`\\.\pipe\...`）がそのまま入る — CLI はどちらも「渡された文字列へ connect する」だけで区別しない。
...
## 9. エラー・互換性・セキュリティ / Windows トランスポート（確定）

- **互換性方針**: docs/hooks-protocol.md §9 と同じ。フィールド追加は後方互換(受信側は未知フィールドを無視、送信側は既知フィールドを欠落させない)。未知の `command` 値は `ok:false` のエラーレスポンスで安全に拒否される(hooks の「未知イベントは黙って破棄」とは異なり、こちらは明示的に伝える — §6)。フレーミング(1 接続 = 1 リクエスト = 1 レスポンス)の変更は破壊的変更とみなし、socketPath/パイプ名の命名規則変更を伴うこと。Windows では **message mode ↔ byte mode の変更もフレーミング変更**とみなす(下記)。
- **セキュリティ**: ソケット/パイプは同一ユーザーのみアクセス可能(AF_UNIX 0600 + 親ディレクトリ 0700 / Named Pipe は下記の same-user DACL)。`tab open` は任意コマンド実行に等しいため、チャネルに書き込めること自体がその LaboLabo インスタンスの全操作権限を意味する(同一ユーザーの別プロセス／別エージェントも含む)。これは hooks チャネルと同じ信頼境界(「同一ユーザーの誰でも」)であり、`plans/012` §2 の「セキュリティ境界」がそのまま適用される。

### 9.1 Windows トランスポート（Named Pipe・実装済み）

Rust 版で実装済み（`labolabo_core::control` の `#[cfg(windows)]` 実装。サーバ = `ControlServer`、クライアント = `send_control_request` — unix と同一シグネチャ）。

- **チャネル**: アプリインスタンスごとに 1 本の Named Pipe。**duplex・message mode**（`PIPE_TYPE_MESSAGE` + `PIPE_READMODE_MESSAGE`）。hooks パイプ（docs/hooks-protocol.md §4.2。byte mode・inbound）とは別名・別モード。
- **パイプ名**: `\\.\pipe\labolabo-control-<アプリ起動時 UUID の先頭 10hex>`（Rust: `control_protocol::control_pipe_name_from_uuid`）。`-control-` の中置が §3 の `control-` プレフィクスに対応し、hooks パイプと字句的に区別される。`LABOLABO_CONTROL_SOCKET`（§4.1）にはこのパイプ名がそのまま入る。
- **アクセス制御**: hooks パイプと同一の same-user DACL（現在ユーザー + SYSTEM に GENERIC_ALL、その他拒否、protected — docs/hooks-protocol.md §4.2/§8）。DACL を構築できなければバインドしない（fail closed）。
- **フレーミング**: **1 接続 = 1 リクエストメッセージ = 1 レスポンスメッセージ**。
  - **half-close の代替（本節の核心）**: Named Pipe には `shutdown(SHUT_WR)` に相当する操作が**存在しない**（`DisconnectNamedPipe`/`CloseHandle` は両方向を同時に終端する）ため、§3 の「書いて half-close → EOF がリクエストの終わり」を直訳できない。byte mode のまま「write 完了後に FlushFileBuffers して読み取りへ移行する」だけでは、flush は送達を保証してもサーバに「リクエストがここで終わり」を伝えられない（サーバの read はさらに続くバイトを待ち続ける）。そこで **message mode のメッセージ境界**を「リクエスト/レスポンスの終わり」として使う — unix の EOF が果たしていた役割を、OS が保存する write 境界で置き換える。
  - 手順: (1) クライアントは接続し、リクエスト JSON 全体を **1 回の書き込み = 1 メッセージ**として送り、`FlushFileBuffers` 後に読み取りへ移行する。(2) サーバは 1 メッセージを読み切り（受信バッファ不足時は `ERROR_MORE_DATA` で分割受信して再組み立て — 受信側の責務。Rust 実装は `recvmsg`/`MsgBuf`）、ハンドラを呼び、レスポンス JSON 全体を 1 メッセージとして書き、flush してから close する（flush しない close はパイプの未読バイトを破棄しうる）。(3) クライアントは 1 メッセージを読む。サーバがレスポンスを書かずに close した場合（`EndOfStream`）は空レスポンスとして扱われ、クライアント側の JSON パース失敗（＝アプリ側エラー、終了コード 1）に落ちる — unix で `read_to_end` が空を返すケースと同じ観測結果。
- **ワイヤ上の JSON**（§6）・**コマンド**（§5）・**CLI の終了コード**（§8）は unix と完全に同一 — 差し替わるのはトランスポート層のみ。
- **接続と切断のレース**: サーバの accept ループは 1 接続ずつ処理する（§3 の「同時実行なし」と同じ）。サーバ多忙時のクライアント接続は `WaitNamedPipe` 相当で待たされ、順番に処理される。
